...
// Neeko+ Gold — Elite ESPN Broadcast Layout (Teams Momentum Pulse)
...
function ProSparkline({ values }: SparkProps) {
  ...
  const { points, lastX, lastY } = React.useMemo(() => {
    ...
    smoothed.forEach((v, i) => {
      const x = smoothed.length === 1 ? 50 : (i / (smoothed.length - 1)) * 100;
  ...
  return (
    ...
    >
      ...
      <svg viewBox="0 0 100 40" className="relative h-full w-full" preserveAspectRatio="none">
        <line x1="0" y1="30" x2="100" y2="30" stroke="rgba(255,255,255,0.2)" strokeWidth="0.7" />
        <line x1="0" y1="22" x2="100" y2="22" stroke="rgba(255,255,255,0.12)" strokeWidth="0.6" />
        <line x1="0" y1="14" x2="100" y2="14" stroke="rgba(255,255,255,0.08)" strokeWidth="0.5" />
...
/* ============================================================================
   2. Metric Insight Card (Gold Frame + Dark Interior)
============================================================================ */
...
function InsightCard({ title, team, metric, values, icon: Icon }: InsightCardProps) {
  return (
    <div className="rounded-2xl bg-[linear-gradient(135deg,#E8C670,#D9A441,#B57C1C)] p-[1.6px] shadow-[0_22px_50px_rgba(0,0,0,0.9)]">
      <div className="relative flex h-full flex-col rounded-[1.1rem] border border-neutral-900/80 bg-gradient-to-b from-[#050505] via-black to-[#020202] p-5">
        ...
        <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-[rgba(232,198,112,0.92)]">
        ...
        <div className="mt-1 inline-flex items-center gap-1 rounded-full border border-[rgba(232,198,112,0.55)] bg-black/80 px-2 py-[4px] shadow-[0_0_16px_rgba(0,0,0,0.8)]">
          ...
          <span className="text-[11px] font-semibold text-neutral-50">{metric}</span>
        </div>
...
/* ============================================================================
   3. Key Headlines Card (New — Matches Players Page)
============================================================================ */

function HeadlinesCard({ items }: { items: string[] }) {
  return (
    <div className="mt-6 rounded-2xl border border-[rgba(232,198,112,0.35)] bg-black/70 p-5 shadow-[0_0_45px_rgba(232,198,112,0.18)]">
...
export default function TeamMomentumPulse() {
  ...
  return (
    <section className="mt-8 rounded-3xl border border-neutral-800/80 bg-[radial-gradient(circle_at_top_left,rgba(232,198,112,0.22),transparent_68%),linear-gradient(to_bottom,#0B0B0D,#000)] px-4 py-10 shadow-[0_0_90px_rgba(0,0,0,0.9)] sm:px-6 md:px-8">
      {/* Header Pill */}
      ...
      <p className="mt-2 max-w-3xl text-sm text-neutral-200">
        Round 23 fantasy trends reveal usage spikes, role changes and matchup edges that shaped
        team performance across the league. This Neeko+ view surfaces the key team-level stories
